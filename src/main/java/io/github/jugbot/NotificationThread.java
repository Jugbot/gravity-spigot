...
public abstract class NotificationThread<T> implements Callable<T> {
  /**
   * An abstract function that children must implement. This function is where all work - typically
   * placed in the run of runnable - should be placed.
   */
  public abstract T doWork();

  /** Our list of listeners to be notified upon thread completion. */
  private java.util.List<TaskListener<T>> listeners =
      Collections.synchronizedList(new ArrayList<TaskListener<T>>());
  ...
  /**
   * Removes a particular listener from this object, or does nothing if the listener is not
   * registered.
   *
  ...
  /**
   * Implementation of the Runnable interface. This function first calls doRun(), then notifies all
   * listeners of completion.
   */
  public T call() {
